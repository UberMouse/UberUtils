...
public class Loot
{
    ...
    private static Filter<GroundItem> paintFilter = new Filter<GroundItem>()
    {
        public boolean accept(GroundItem groundItem) {
            return true;
        }
    ...
    public static void takeLoot(Filter<GroundItem> filter) {
        ...
        //    if (g.getGroundItem().getDefinition(). == 1 && UberInventory.contains(g.getItem().getId()))
        //        stackable = true;
        int count = 0;
        try {
        ...
        if (loot.validate()) {
            ...
            if (loot.interact("Take " + name)) {
                ...
                if (PriceThread.priceForId(id) > 0)
                    add = PriceThread.priceForId(id) * 1;//;loot.getItem().getStackSize();
                totalPrice += add;
    ...
    public static void takeLoot(final int... ids) {
        takeLoot(new Filter<GroundItem>()
        {
            public boolean accept(GroundItem t) {
                return Utils.arrayContains(ids, t.getId());
            }
    ...
    public static void repaint(Graphics2D g) {
        GroundItem[] items = GroundItems.getLoaded(paintFilter);
        ...
        for (Tile tile : tiles) {
            ...
            for (GroundItem item : GroundItems.getLoadedAt(tile.getX(), tile.getY())) {
                if (Utils.arrayContains(ids, item.getId())) {
    ...
    public static void lootInfo(Graphics2D g, GroundItem loot, int offset) {
        if (!loot.isOnScreen())
            return;
        final Item groundItem = loot.getGroundItem();
        String lootString = groundItem.getName() + " (" + groundItem.getStackSize() + " * ";
        lootString += (PriceThread.priceForId(groundItem.getId()) == -1) ?
                      "Calculating.." :
                      (PriceThread.priceForId(groundItem.getId()) == -2) ?
                      "Unknown" :
                      PriceThread.priceForId(groundItem.getId());
        lootString += ")";
        ...
        final Tile location = loot.getLocation();
        final Point point = Calculations.groundToScreen(location.getX(), location.getX(), location.getPlane(), 0);
    ...
    public static boolean isPaintValid() {
        return GroundItems.getLoaded(paintFilter).length > 0;
    }
    ...
    public static void setPaintFilter(Filter<GroundItem> filter) {
    ...
    public static boolean shouldLoot() {
        boolean stackable = false;
        //for (GroundItem g : GroundItems.getLoaded(filter))
        //    if (g.getItem().getDefinition().getStackType() == 1 && UberInventory.contains(g.getItem().getName()))
        //        stackable = true;
        int count = 0;
        try {
            for (Item i : UberInventory.getItems())
