...
public class UberPlayer
{
    ...
    public static Character interacting() {
        return UberPlayer.get().getInteracting();
    }
    ...
    public static void drinkPotions() {
        ...
        for (Potions potion : Potions.values())
            if (!Utils.isBoosted(potion.skill()) && UberInventory.contains(potion.getName())) {
                UberInventory.getItem(potion.getName()).getWidgetChild().click(true);
    ...
    public static NPC getInteractor() {
        return NPCs.getNearest(new Filter<NPC>() {
            public boolean accept(NPC npc) {
                return npc.getInteracting() != null && npc.getInteracting().equals(get());
            }
        });
    }
}
